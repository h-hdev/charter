      ...
      drawPoints: function () {
        ...
        points.forEach((p) => {
          ...
          if (!p.options.dataLabels || p.options.dataLabels.enabled !== false) {
            ...
            let dlText = this.options.dataLabels.format
              ? H.format.call(
                  p,
                  this.options.dataLabels.format,
                  { point: p },
                  chart,
                )
              : this.options.dataLabels.formatter.call(p, p);

            const dataLabel = renderer
              .text(dlText)
              .attr({
                class: "highcharts-label highcharts-arc-label",
                x: 0,
                y: 0,
                translateX: p.plotX,
                translateY: p.plotY,
