    ...
    },
    {
      ...
      translate: function () {
        ...
        points.forEach((p) => {
          ...
          const xAngle =
            this._getXAngle(this._getDataLabelX(p, dataLabels, xAxis.dataMin)) +
          ...
          p.plotX = pos.x + (dataLabels.x || 0); // - chart.plotLeft;
          p.plotY = pos.y; // - chart.plotTop;
      ...
      drawPoints: function () {
        ...
        points.forEach((p) => {
          ...
          if (p.options.linker) {
            let linkerPath = this.getYLinkPath(
              p.x + (p.x1 - p.x) / 2,
              ...p.options.linker,
            );
