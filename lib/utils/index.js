...
const Utils = {
  ...
  merge: (source, target) => {
    Object.keys(target).forEach((key) => {
      if (!source[key]) {
        ...
        Utils.merge(source[key], target[key]);
      } else {
        source[key] = target[key];
      }
    });
  ...
  isArr: (what) => {
    return (
      what &&
      typeof what === "object" &&
      what.constructor.toString().includes("Array")
    );
  },
  ...
  isIndex: (what, length) => {
    ...
    return (
      !!length &&
      (type === "number" || (type !== "symbol" && reIsUint.test(what))) &&
      what > -1 &&
      what % 1 === 0 &&
      what < length
    );
  ...
  set(obj, key, value, separator = ".") {
    ...
    while (parent !== null && ++index < length) {
      ...
      if (index !== lastIndex) {
        objValue = parent[k];
        newValue = Utils.isObj(objValue)
          ? objValue
          : Utils.isIndex(path[index + 1])
            ? []
            : {};
      }
